...
import { EnforcementEngine } from "@/features/ironman";
...
export function WebPlayerProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    spotifyClient.search.searchTracks(`track:${lockInTrackId}`)
      .then((tracks: SpotifyTrack[]) => {
        const match = tracks.find((t) => t.id === lockInTrackId);
        if (match) playTrack(match);
      })
      .catch(() => {});
  }, [canControlPlayback, spotifyClient, session, playTrack, searchParams]);

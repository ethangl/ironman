...
export function IronmanPanel({
      ...
      fetch("/api/ironman/status")
        ...
        .then((data) => {
          if (data && data.count !== prevCountRef.current) {
            ...
            if (hit) {
              toast.success(`${hit.badge} ${hit.label}`, {
                description: `${hit.threshold} plays reached`,
              });
            }
  ...
  return (
    <div className="flex flex-col items-center gap-8 py-8">
      ...
      <Link
        href={`/song/${initialStreak.trackId}`}
        className="text-center hover:opacity-80 transition"
      >
        <h2 className="text-2xl font-bold">{initialStreak.trackName}</h2>
        <p className="text-muted-foreground">{initialStreak.trackArtist}</p>
      </Link>
      ...
      <div className="gap-8 grid lg:grid-cols-2 self-stretch space-y-12">
        <div className="space-y-12">
          ...
          <Leaderboard trackId={initialStreak.trackId} liveCount={count} />
        </div>

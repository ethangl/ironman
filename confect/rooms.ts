...
function normalizeQueuedTrack(input: {
  ...
  return {
    ...
    trackId,
    // Conditional rather than `trackImageUrl: input.trackImageUrl` so the field
    // is absent (not `undefined`) when missing — required under
    // exactOptionalPropertyTypes where this value spreads into optional fields.

...
export const PlayerWrapper: FC<PropsWithChildren> = ({ children }) => {
  ...
  return (
    <section
      className={cn(
        "grid overflow-hidden relative rounded-3xl select-none transition-all",
        isPlaying
        ...
        <div className="absolute pointer-events-none inset-0 z-0">
          ...
          {displayImage && (
            <img
              src={displayImage}
              alt=""
              className="absolute blur-sm object-cover opacity-40 scale-200 size-full -translate-y-[25%]"
            />
          )}
        </div>
        {children}
